...
# this is extremely unoptimized, and I feel dirty about it, but hey, it works... slowly
        ...
        for connection in self.getConnections(currentNode):
            ...
            for cave in set(visited):
                if visited.count(cave) == 2:
                    ...
                    break
            if connection == 'end':
                currentPathCopy = currentPath.copy()
                currentPathCopy.append('end')
                if currentPathCopy not in paths:
                    paths.append(currentPathCopy)
            elif connection == 'start':
                continue
            elif not anySmallCaveVisitedtwice or connection not in visited:
